...
bool Pn532I2c::firmwareVersion(uint32_t& version) {
  ...
  if (!command(CMD_GET_FIRMWARE_VERSION, nullptr, 0, response, length,
               sizeof(response)) || length != sizeof(response))
    return false;
...
bool Pn532I2c::scan(uint16_t timeoutMs, uint8_t* uid, uint8_t& uidLength) {
  ...
  if (!command(CMD_IN_LIST_PASSIVE_TARGET, arguments, sizeof(arguments),
               response, length, sizeof(response), timeoutMs))
    return false;
  ...
  if ((reportedLength != 4 && reportedLength != 7) ||
      static_cast<uint8_t>(6 + reportedLength) > length)
    return false;
  std::memcpy(uid, response + 6, reportedLength);
...
bool Pn532I2c::authenticate(const uint8_t* uid, uint8_t uidLength,
  ...
  arguments[2] = block;
  std::memcpy(arguments + 3, key, 6);
  // MIFARE Classic authentication uses the last four UID bytes.
  std::memcpy(arguments + 9, uid + uidLength - 4, 4);
  uint8_t response[1]{};
  uint8_t length = 0;
  return command(CMD_IN_DATA_EXCHANGE, arguments, 13, response, length,
                 sizeof(response)) && length == 1 && response[0] == 0;
}
...
bool Pn532I2c::writeFrame(
  ...
  for (uint8_t i = 0; i < argumentLength; ++i) {
    wire_->write(arguments[i]);
    checksum = static_cast<uint8_t>(checksum + arguments[i]);
  }
...
bool Pn532I2c::readAck() {
  ...
  return readI2c(frame, sizeof(frame)) &&
         std::memcmp(frame, ACK, sizeof(ACK)) == 0;
}

bool Pn532I2c::readResponse(uint8_t commandByte, uint8_t* response,
                            uint8_t& responseLength, uint8_t responseCapacity) {
  uint8_t frame[31]{};
