...
bool Dfr0534Manager::begin(uint8_t rxPin, uint8_t txPin, int8_t busyPin) {
  if (!board::canReadDigital(rxPin) || !board::canWriteDigital(txPin) ||
      ...
      rxPin == txPin ||
      (busyPin >= 0 && !board::canReadDigital(static_cast<uint8_t>(busyPin)))) return false;
  reset();
...
bool Dfr0534Manager::send(uint8_t command, const uint8_t* data, uint8_t length) {
...
bool Dfr0534Manager::playTrack(uint16_t track) {
  ...
  const uint8_t data[] = {static_cast<uint8_t>(track >> 8), static_cast<uint8_t>(track)};
...
bool Dfr0534Manager::readStatus(uint8_t& status, uint32_t timeoutMs) {
  while (serial_ != nullptr && serial_->available()) serial_->read();
