...
bool NeoPixelManager::begin(uint8_t pin, uint16_t count, uint8_t brightness) {
  if (!board::canWriteDigital(pin) || count == 0 || count > 1024) return false;
  reset();
...
bool NeoPixelManager::set(uint16_t index, uint8_t red, uint8_t green, uint8_t blue,
                          uint8_t white) {
...
bool NeoPixelManager::fill(uint16_t first, uint16_t count, uint8_t red, uint8_t green,
